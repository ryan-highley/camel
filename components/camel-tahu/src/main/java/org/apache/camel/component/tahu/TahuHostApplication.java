...
public class TahuHostApplication extends HostApplication {
    ...
    static class HostApplicationBuilder {
        ...
        HostApplicationBuilder hostId(String hostId) {
            checkBuildState();
            this.hostId = hostId;
            return this;
        }

        HostApplicationBuilder sparkplugSubscriptions(List<String> sparkplugSubscriptions) {
            checkBuildState();
            this.sparkplugSubscriptions = List.copyOf(sparkplugSubscriptions);
            return this;
        }

        HostApplicationBuilder serverDefinitions(List<MqttServerDefinition> serverDefinitions) {
            checkBuildState();
            this.serverDefinitions = List.copyOf(serverDefinitions);
            return this;
        }

        HostApplicationBuilder onlineState(boolean onlineState) {
            checkBuildState();
            this.onlineState = onlineState;
            return this;
        }

        HostApplicationBuilder onMessageConsumer(BiConsumer<EdgeNodeDescriptor, Message> onMessageConsumer) {
            checkBuildState();
            this.onMessageConsumer = onMessageConsumer;
            return this;
        }

        HostApplicationBuilder onMetricConsumer(BiConsumer<EdgeNodeDescriptor, Metric> onMetricConsumer) {
            checkBuildState();
